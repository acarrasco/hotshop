...
app.post('/user/:userId/dismissQuestions', function (req, res) {
    db.updateQuestionsDate(req.params.userId, Math.round(Date.now() / 1000), function (err) {
        if (err) {
            return res.json({Error: '' + err, reason: 'db error dmissing questions'});
        }
        res.json({Error: false});
    });
});

app.post('/user/:userId/dismissReviews', function (req, res) {
    db.updateReviewsDate(req.params.userId, Math.round(Date.now() / 1000), function (err) {
        if (err) {
            return res.json({Error: '' + err, reason: 'db error dmissing reviews'});
        }
        res.json({Error: false});
    });
});
